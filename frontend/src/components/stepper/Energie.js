...
export default function Energie({ setFormData, FormData }) {
  ...
  const [inputs1, setInputs1] = useState([
    { id: 1, label: "Electricité", img: "/Lightning Bolt.png" },
    { id: 2, label: "Combustibles", img: "/Gas Industry.png" },
  ]);
  const [categories, setcategories] = useState(["Electricité", "Combustibles"]);
  ...
  const processDocuments = (docs) => {
    ...
    const groupedDocs = docs.reduce((acc, doc) => {
      ...
      if (existingDoc) {
        existingDoc.details.push({
          Type_poste: doc.Type_poste,
          Post: doc.Post,
        });
      } else {
        acc.push({
  ...
  const handleClick = (item) => {
    if (isElement) {
      selectItem(item);
      togglePopup();
      setcategories(["Electricité", "Combustibles"]);
      settags([]);
      setElement(false);
    } else {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.post(
          "http://localhost:8080/api/calcul/getSubCategory",
          { tags }
        );
        console.log(tags);
        // Handle the response
        if (response.data.error === "Thats all the tags there") {
          ...
          console.log("fff");
          const responseFromGetDocumentsWithTags = await axios.post(
            "http://localhost:8080/api/calcul/getDocsWithTags",
            { tags }
          );
          const processedDocs = processDocuments(
            responseFromGetDocumentsWithTags.data.docs
          );
  ...
  return (
    <div className="w-[60%] aileron flex flex-col gap-6 items-center pt-4 mx-auto">
      {inputs.map((input) => (
        <div
          key={input.id}
          className="mb-4 w-full items-center justify-center flex"
        >
          <label htmlFor={`input${input.id}`} className="mr-4">
            {input.label}
          </label>
      ...
      <button
        onClick={togglePopup}
        className="flex gap-3 px-6 py-2 bg-[#FFFAF6] border-[#FD8D14] border-[1px] rounded-lg"
      >
        <img src="/Union.svg" alt="plus"></img>
        <span className="text-[#FD8D14] aileron">Ajouter une ligne</span>
      </button>
      {showPopup && (
        <div className="fixed overflow-y-auto inset-0 flex items-center justify-center bg-gray-500 bg-opacity-50 z-20">
          <div className="bg-[#CFCF5C] flex items-center pt-16 rounded-lg flex-col w-[70%] p-4 rounded shadow-md">
            <input
              className="rounded-[20px] pl-4 p-2"
            ...
            <ul className="flex flex-col items-center w-full mt-16 gap-6">
              {categories.map((item, index) => (
                <li
                  key={index}
                  className="cursor-pointer justify-between px-4 w-[90%] flex gap-4 items-center py-2 bg-[#FFFAF6] hover:bg-gray-200 p-2 flex rounded"
                  onClick={() => handleClick(item)}
                >
                  <div className="flex gap-4">
                    <img src="/Trilobite.png" alt="desc"></img>

                    <span className="aileron text-[#704116]">
                      {" "}
                      {typeof item === "object" ? item.Nom : item}{" "}
                      {typeof item === "object" ? item.Unity : ""}{" "}
                    </span>
                  </div>
                  <div>
                    <svg
                    ...
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M6.34375 8.5V14H9.34375V8.5H14.8438V5.5H9.34375V0H6.34375V5.5H0.84375V8.5H6.34375Z"
                        fill="#704116"
                      />
                    </svg>
                  </div>
